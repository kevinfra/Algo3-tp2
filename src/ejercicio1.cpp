...
void mirarDerechaYAbajo(int f, int c, int p, int u, int nivelActual, int i, int k, char derecha, char abajo, Grafos::ListaAdy &grafo){
  if (abajo == '#'){
    if (nivelActual <= p) grafo.agregarArista(u, ( (f*i+k+1)+(f*c*(nivelActual+1)) ));
  }
  else{
    grafo.agregarArista(u, ((f*i+k+1)+(f*c*nivelActual)) );
  }

  if (derecha == '#') {
    if (nivelActual <= p) grafo.agregarArista(u, ((f*(i+1)+k)+(f*c*(nivelActual+1)) ));
  }
  else{
    grafo.agregarArista(u, ((f*(i+1)+k)+(f*c*nivelActual)) );
  }
}
...
int parserEj1(int f, int c, int p, Grafos::ListaAdy &grafo, int &s, int &t){
  ...
  for (int nivelActual = 0; nivelActual < p; ++nivelActual){
    for (int i = 0; i < c; ++i){
      for (int k = 0; k < f; ++k){
        int nodoActual = f*i+k+nivelActual;
        if (i == 0){
          if (k == 0){
